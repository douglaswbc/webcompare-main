...
const InputField = ({ label, value, onChange, type = "text", colSpan = 1, placeholder = "" }: any) => (
  <div className={`col-span-1 md:col-span-${colSpan}`}>
    ...
    <input
      type={type}
      className="w-full bg-slate-900 text-white p-2.5 rounded border border-slate-700 focus:border-primary outline-none transition-colors"
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white">Gerenciar Planos e Provedores</h1>
        <div className="flex gap-2">
          <button
            onClick={() => setView('plans')}
            className={`px-4 py-2 rounded transition-colors ${view === 'plans' ? 'bg-primary text-white' : 'text-slate-400 hover:text-white'}`}
          >
            Planos
          </button>
          <button
            onClick={() => setView('providers')}
            className={`px-4 py-2 rounded transition-colors ${view === 'providers' ? 'bg-primary text-white' : 'text-slate-400 hover:text-white'}`}
          >
            Provedores
          </button>
          <button
            onClick={() => setView('import_ceps')}
            className={`px-4 py-2 rounded transition-colors ${view === 'import_ceps' ? 'bg-primary text-white' : 'text-slate-400 hover:text-white'}`}
          >
            Importar CEPs
          </button>
          {view !== 'import_ceps' && (
            <button
              onClick={() => openModal()}
              className="bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded flex items-center gap-2 transition-colors ml-4"
            >
      ...
      {view === 'import_ceps' && (
        <div className="bg-background-paper-dark p-6 rounded-xl border border-white/10 max-w-2xl mx-auto">
          <h3 className="text-lg font-bold text-white mb-4">Importação em Massa de CEPs</h3>
              ...
              <tbody>
                {(view === 'plans' ? plans : providers).map((item: any) => (
                  <tr key={item.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    {view === 'plans' && <td className="p-4 font-mono text-primary">R$ {item.price}</td>}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-background-paper-dark w-full max-w-4xl rounded-xl border border-white/10 p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {view === 'plans' && (
                <>
                  <div className="col-span-1 md:col-span-4 text-primary font-bold text-sm border-b border-white/5 pb-1 mt-2">DADOS GERAIS</div>
                  ...
                  <div className="col-span-1 md:col-span-2">
                    ...
                    <select className="w-full bg-slate-900 text-white p-2.5 rounded border border-slate-700 focus:border-primary outline-none"
                  ...
                  <div className="col-span-1 md:col-span-4 text-primary font-bold text-sm border-b border-white/5 pb-1 mt-4">ESPECIFICAÇÕES</div>
                  ...
                  <div className="col-span-1 md:col-span-4 text-primary font-bold text-sm border-b border-white/5 pb-1 mt-4">VISUAL</div>
            ...
            <div className="flex gap-2 mt-8 justify-end pt-4 border-t border-white/10">
              ...
              <button onClick={handleSave} className="bg-primary px-8 py-2 rounded text-white font-bold hover:bg-primary-hover">Salvar</button>
            </div>
          </div>
        </div>
      )}
