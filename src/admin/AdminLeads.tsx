  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-4">
          ...
          {/* Barra de Filtros */}
          <div className="flex flex-col md:flex-row items-center gap-4 bg-[#192633] p-4 rounded-xl border border-white/10">
             
             {/* Filtro Nome */}
             <div className="flex items-center gap-2 w-full md:w-auto flex-1 bg-[#0d141c] p-2 rounded border border-slate-700">
                    ...
                    <tr key={lead.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="p-4">
                            <div className="text-white flex items-center gap-1">
                                <span className="material-symbols-outlined text-xs text-green-500">smartphone</span>
                                {lead.phone}
                            </div>
                            <div className="text-xs opacity-70 mt-1 pl-4">{lead.cpf}</div>
                        </td>
                        <td className="p-4 text-[#0096C7] font-medium">
                            <span className="bg-[#0096C7]/10 px-2 py-1 rounded border border-[#0096C7]/20 whitespace-nowrap">
                                {lead.plans?.name || 'Plano Desconhecido'}
                            </span>
                        </td>
                        <td className="p-4 max-w-xs truncate text-xs" title={JSON.stringify(lead.address_json)}>
                            {lead.address_json?.logradouro ? (
                                <>
                                    <p className="text-white mb-0.5">{lead.address_json.logradouro}, {lead.address_json.numero}</p>
                                    <p>{lead.address_json.bairro} - {lead.address_json.localidade}/{lead.address_json.uf}</p>
                                </>
