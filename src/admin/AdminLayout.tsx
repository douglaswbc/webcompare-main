...
const AdminLayout: React.FC = () => {
  ...
  if (!session) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background-dark p-4">
        <form onSubmit={handleLogin} className="w-full max-w-md bg-background-paper-dark p-8 rounded-xl border border-white/10 shadow-2xl">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full bg-slate-900 text-white p-3 rounded border border-slate-700 focus:border-primary outline-none transition-colors"
            />
          </div>
          <div className="mb-6">
          ...
          <button type="submit" className="w-full bg-primary hover:bg-primary-hover text-white p-3 rounded font-bold transition-colors">
            Entrar
          </button>
        </form>
      </div>
  ...
  return (
    <div className="flex min-h-screen bg-background-dark relative overflow-hidden">
      ...
      <div className="md:hidden fixed top-0 w-full bg-background-paper-dark border-b border-white/10 z-30 px-4 py-3 flex items-center justify-between">
...
const NavLink = ({ to, icon, label }: any) => {
  ...
  return (
    <Link
      to={to}
      className={`flex items-center gap-3 p-3 rounded transition-colors ${isActive ? 'bg-primary text-white font-medium' : 'text-slate-300 hover:text-white hover:bg-white/5'}`}
    >
