                ...
                <div className="flex-1 text-left">
                    ...
                    <ul className="space-y-4">
                        {[
                            ...
                            <li key={i} className="flex items-start gap-3 text-text-inverted/70">
                                <span className="material-symbols-outlined text-primary mt-0.5">check_circle</span>
                                {item}
                            </li>
                        ))}
                    </ul>
                    <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="mt-8 text-text-inverted border border-text-inverted/30 hover:bg-text-inverted/10 px-6 py-3 rounded-xl font-bold transition-all">
                        Comparar Agora
                    </button>
                </div>
                <div className="flex-1">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-background-paper-dark p-6 rounded-2xl border border-background-dark transform translate-y-8">
                            <span className="text-4xl font-black text-text-inverted block mb-1">50k+</span>
                            <span className="text-sm text-text-muted">Comparisons realizadas</span>
                        </div>
                        ...
                        <div className="bg-background-paper-dark p-6 rounded-2xl border border-background-dark col-span-2">
                            <div className="flex items-center gap-1 text-accent mb-2">
                                {[1, 2, 3, 4, 5].map(s => <span key={s} className="material-symbols-outlined text-sm">star</span>)}
                            </div>
                            <p className="text-text-inverted italic text-sm">"Encontrei um plano com o dobro da velocidade pagando menos do que eu pagava na operadora antiga."</p>
                            <p className="text-xs text-text-muted mt-2 font-bold">- Roberto A., São Paulo</p>
                        </div>
                    </div>
                </div>
            </div>
